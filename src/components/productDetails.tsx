...
const ProductDetails: React.FC = () => {
  ...
  const product = useSelector((state: RootState) =>
    selectProductDetails(state)
  );
  const status = useSelector((state: RootState) =>
    selectProductDetailsStatus(state)
  );
  const error = useSelector((state: RootState) =>
    selectProductDetailsError(state)
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-blue-400">
      ...
      <p className="text-xl font-semibold text-white mb-2">${product.price}</p>
      ...
      <button
        className="text-white font-bold p-2 bg-blue-300 mt-4 rounded-md cursor-pointer"
